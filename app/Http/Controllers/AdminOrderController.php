...
{
    $ordersQuery = Order::with('user'); // Gunakan nama variabel yang berbeda
    ...
    if ($request->has('tanggal_pesanan') && $request->tanggal_pesanan != '') {
        $ordersQuery->whereDate('created_at', $request->tanggal_pesanan);
    }

    // Filter berdasarkan pelanggan
    if ($request->has('pelanggan') && $request->pelanggan != '') {
        $ordersQuery->whereHas('user', function ($query) use ($request) {
            $query->where('name', 'like', '%' . $request->pelanggan . '%');
        });
    }

    $orders = $ordersQuery->latest()->paginate(10); // Urutkan dan paginasi
